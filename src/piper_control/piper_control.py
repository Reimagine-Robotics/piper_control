...
class ArmOrientations:
  ...
  def from_string(cls, orientation_name: str) -> ArmOrientation:
    ...
    available = [o.name for o in cls.__dict__.values() if isinstance(o, ArmOrientation)]
    raise ValueError(
...
class BuiltinJointPositionController(JointPositionController):
  ...
  def __init__(
    ...
    rest_position: (Sequence[float] | None) = ArmOrientations.upright.rest_position,
  ):
...
class MitJointPositionController(JointPositionController):
  ...
  def __init__(
    ...
    try:
      ...
      raise ValueError(f"Invalid firmware version string: {current_firmware}") from e
